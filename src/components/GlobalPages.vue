<template>
  <div class="h-screen w-full flex antialiased bg-gray-200 dark:text-gray-200 dark:bg-gray-900 overflow-hidden">
    <div class="flex-1 flex flex-col">
      ...
      <!-- Top header -->
        <main class="flex-grow flex flex-row min-h-0">
          <!-- Left Section -->
          <ChatLeftsection @parsing="show = $event" class=""></ChatLeftsection>
          <Teams v-show="isadmin" v-if="hash === '#team'" class="md:w-11/12 w-full"></Teams>
          <admindashboard v-show="isadmin" v-else class="w-full md:w-11/12 sm:w-8/12"></admindashboard>
            <!-- Sidebar -->
          <aside
            class="h-full w-16 flex flex-col space-y-10 items-center justify-center relative bg-gray-800 text-white">
            <!-- Profile -->
            <router-link to="/dashboard">
            <div v-on:click="select('dash')" :class="dashclass"
            ...
            <!-- Courses -->
            <div v-on:click="select('team')" :class="{active : true}"
              class="h-10 w-10 flex items-center justify-center rounded-lg cursor-pointer hover:text-gray-800 hover:bg-white  hover:duration-300 hover:ease-linear focus:bg-white">
  ...
  export default {
    ...
    methods: {
      select(event){
        switch(event){
          case 'dash':
            this.dashclass = 'bg:white'
            this.teamsclass = ''
          case 'team':
            this.dashclass = ''
            this.teamsclass = 'bg:white'
        }
      }
    },
  }
