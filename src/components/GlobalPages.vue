...
import Setting from './parsial/admin/Setting.vue';
import Dashboarduser from './base/dashboarduser.vue';
  export default {
    mixins : [whois],
    name: 'GlobalPage',
    data(){
    ...
    watch:{
        $route (to, from){
            this.hash = to.hash
            this.select(this.syncview(location.hash))
        }
    },
    props: {
      msg: String
    ...
    mounted() {
      ...
      // this.eventbottomnav(this.syncview(location.hash)-1)
      this.select(this.syncview(location.hash));      
      // console.log(this.syncview(location.hash))
    },
    methods: {
      syncview(loc){
        let hash = ['','#team','#info','#profil']
        return hash.indexOf(loc)
      },
      eventbottomnav(value){
        if(this.posbottomnav != value){
          switch(value){
            case 0 :
              this.leftsection = true
              this.maincontent = false
              if(this.posbottomnav != 1) this.$router.push('/dashboard')
              break;
            case 1 :
              this.leftsection = false
              this.maincontent = true
              this.select(0)
              if(this.posbottomnav != 0) this.$router.push('/dashboard')
              break;
            case 2 :
              this.leftsection = false
              this.maincontent = true
              this.select(3)
              if(this.posbottomnav != 2) this.$router.push('/dashboard#profil')
              break;
          }
        this.posbottomnav = value
        }
      },
      select(event){
        switch(event){
